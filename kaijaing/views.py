...
from kaijaing.models import User,quanxian,Color,SkuType,SKU,Wuliu,Wenzi,Order,Order_Del,packing,Roles,Roles_Del,Size,Style,Pack_method,Draw_Pi,Warehousing,Warehous_Del,Contacts,cl_SKU,Stock,Warehous_Del_Out,Warehousing_Out
...
from itertools import chain  #查询结果合并的函数库
    ...
    if user_radio == '3': #标识全部订单
        if selectForm.get('input') == '' and selectForm.get('date') == '' and selectForm.get('draw_status') == '' and selectForm.get('status') == '':
            ...
            if selectForm.get('input'):
                order_del_list = Order_Del.objects.filter(order_code=selectForm.get('input')).values('order_key')  #查询ERP单号
                if order_del_list:  #如果ERP单号查询存在
                    order_key = order_del_list[0].get('order_key')
                    user_list = user_list.filter(Q(id__icontains=selectForm.get('input'))|Q(id=order_key)|Q(date__icontains=selectForm.get('input'))|Q(add__icontains=selectForm.get('input'))|Q(salesman__icontains=selectForm.get('input')))
                else:
...
def sku(request):    #获取sku信息
    ...
    user_json = []
    user_data={}
    page_price=post_data.get('page_price')  #包装单价
    draw_price=post_data.get('draw_price')  #画图单价状态
    if post_data.get('serch') == '' or post_data.get('serch') is None:
    ...
    if page_price:  # 存在单价
        if page_price == '为0':
            user_list = user_list.filter(price=0)
        else:
            user_list = user_list.filter(price__gt=0)
    if draw_price:  # 如果存在画图单价
        if draw_price == '为0':
            user_list = user_list.filter(draw_price=0)
        else:
            user_list = user_list.filter(draw_price__gt=0)
    user_page=Paginator(user_list,post_data.get('size')).page(post_data.get('page'))
    total= Paginator(user_list,post_data.get('size')).count    #总计的数量
    for user_i in user_page:
        user_json.append(user_i)
...
def cl_order_sku(request):    #获取材料sku信息
    ...
    for user_i in user_page:
        user_json.append(user_i)
    return JsonResponse(data=user_json,safe=False)
def cl_out_sku(request):    #获取出库获取库存SKU明细
    post_data = request.body
    post_data = json.loads(post_data)
    user_json = []
    if post_data.get('serch'):
        ...
        user_list = Stock.objects.filter().values()
    user_page = Paginator(user_list, 30).page(1)
    for user_i in user_page:
        user_json.append(user_i)
    return JsonResponse(data=user_json,safe=False)
...
def sku_up_select(request):
    ...
    try:
        ...
        for user_i in user_list :
            ...
            user_json['technology']=user_i.get('technology')
        return JsonResponse(data=user_json,safe=False)
    except:
        return HttpResponse('NOT OK')
def sku_update(request):
    ...
    SKU.objects.filter(id=post_data.get('id')).update(name=post_data.get('name'),type=post_data.get('type'),unit=post_data.get('unit'),barcode=post_data.get('barcode'),imageUrl=post_data.get('imageUrl'),
                                                      price=post_data.get('price'),draw_price=post_data.get('draw_price'),cost_price=post_data.get('cost_price'),style_num=post_data.get('style_num'),technology=post_data.get('technology'))
    return HttpResponse('OK')
    #except:
        #return HttpResponse('NOT OK')
...
def order_save(request):
    ...
    amount = 0 #金额汇总
    biaozhi = 0  #用来标记是否已经有ERP单号的订单保存过了
    order_shen_list=[]
    for order_i in order_shen:
        sku_img=order_i.get('picture')
        sku_pic=SKU.objects.filter(id=order_i.get('item_code')).values('imageUrl')  #获得最新的图片信息
        if sku_pic:
            if sku_pic[0]:
                sku_img = sku_pic[0].get('imageUrl')
        #判断ERP单号是否在别的订单里面已经存在了
        num_new = order_i.get('total_num')  #目前的合数
        if Order_Del.objects.filter(order_code=order_i.get('order_code'),state='未审核'): #如果数据已经存在这个订单了
            num = 0 #初始化
            order_list = Order_Del.objects.filter(order_code=order_i.get('order_code'),state='未审核').values('num')
            for order_list_i in order_list:
                num = num + order_list_i.get('num')
            num_new = num_new + num
            #更新已有订单的合数
            Order_Del.objects.filter(order_code=order_i.get('order_code'),state='未审核').update(total_num = num_new)
            biaozhi = 1
        order_shen_i=Order_Del(order_key=order_tou.get('order_code'),order_code=order_i.get('order_code'),item_code=order_i.get('item_code'),item_name=order_i.get('item_name'),order_name=order_i.get('order_name'),
                                  ...
                                  wuliu=order_i.get('wuliu'),pack_method=order_i.get('pack_method'),note=order_i.get('note'),size=order_i.get('size'),total_num=num_new,draw_amount=order_i.get('draw_amount'),
                                  ...
                                  order_level=order_i.get('order_level'),amount=order_i.get('amount'),order_img=order_i.get('order_img'),picture=sku_img,salesman=order_tou.get('saleman'),date=order_tou.get('date'))
        order_level= order_i.get('order_level')
        order_shen_list.append(order_shen_i)
        amount = amount +float(order_i.get('amount'))   #总金额
        amount = round(amount,2)
    Order_Del.objects.bulk_create(order_shen_list)  #单身批量保存
    order_tou_save = Order(id=order_tou.get('order_code'), date=order_tou.get('date'), add=order_tou.get('add'),total_amount=amount,
...
def cl_order_save(request):
    ...
    Warehousing.objects.filter(id=order_tou.get('order_code')).delete()  # 删除单头
    Warehous_Del.objects.filter(order_key=order_tou.get('order_code')).delete()  # 删除单身

    order_tou_save=Warehousing(id=order_tou.get('order_code'),date=order_tou.get('date'),note=order_tou.get('note'),supplier_ware=order_tou.get('supplier_ware'),create_time=date,create_user=order_tou.get('create_user'),type='入库')
    order_shen_list = []
    for order_i in order_shen:
        ...
        order_shen_list.append(order_shen_save)
    order_tou_save.save()
    Warehous_Del.objects.bulk_create(order_shen_list)
    return HttpResponse('OK')
def clout_order_save(request):
    date = time.strftime('%Y-%m-%d %H:%M', time.localtime(time.time()))  # 获取当前时间
    post_data=request.body  #获得页面数据
    post_data=json.loads(post_data)
    order_tou=post_data.get('order_tou')
    ...
    Warehousing_Out.objects.filter(id=order_tou.get('order_code')).delete()  # 删除单头
    Warehous_Del_Out.objects.filter(order_key=order_tou.get('order_code')).delete()  # 删除单身

    order_tou_save=Warehousing_Out(id=order_tou.get('order_code'),date=order_tou.get('date'),note=order_tou.get('note'),create_time=date,create_user=order_tou.get('create_user'))
    order_shen_list = []
    for order_i in order_shen:
        #比较库存和提交的单据情况
        item_code=order_i.get('item_code')
        kucun = Stock.objects.filter(item_code=item_code).values('stock_num')
        kucun = kucun[0].get('stock_num')
        if kucun >= int(order_i.get('num')) :  #库存满足
            order_shen_save=Warehous_Del_Out(order_key=order_tou.get('order_code'),item_code=order_i.get('item_code'),item_name=order_i.get('item_name'),format=order_i.get('format'),
                                      unit=order_i.get('unit'),num=order_i.get('num'),note=order_i.get('note'),create_user=order_tou.get('create_user'),state='未审核',date=order_tou.get('date'))
        else: #库存不足
            return HttpResponse(order_i.get('item_name'))
        order_shen_list.append(order_shen_save)
    order_tou_save.save()
    Warehous_Del_Out.objects.bulk_create(order_shen_list)
    #扣减库存
    for order_i in order_shen:
        # 比较库存和提交的单据情况
        item_code = order_i.get('item_code')
        kucun = Stock.objects.filter(item_code=item_code).values('stock_num')
        kucun = kucun[0].get('stock_num')
        kucun = kucun - int(order_i.get('num'))
        Stock.objects.filter(item_code=item_code).update(stock_num=kucun)
    return HttpResponse('OK')
def cl_order_update(request):
    ...
    #2.重新保存内容
    order_tou_save=Warehousing(id=order_tou.get('id'),date=order_tou.get('date'),note=order_tou.get('note'),supplier_ware=order_tou.get('supplier_ware'),create_time=date,create_user=order_tou.get('create_user'),type='入库')
    order_shen_list = []
    for order_i in order_shen:
...
def order_update(request):
    ...
    #删除前台删除的内容,此处不能直接删除,之前的状态,比如已经画图,已经
    #Order_Del.objects.filter(order_key=order_tou.get('id')).delete()
    #修改后,先删除前台删除的明细
    for detele_i in order_delete:  #循环删除的明细
        Order_Del.objects.filter(id=detele_i).delete()
    total_num = 0 #单头合计
    order_shen_list = []
    amount = 0
    order_level=''
    for order_i in order_shen:
        ...
        if sku_pic:
            if sku_pic[0]:
                sku_img = sku_pic[0].get('imageUrl')
        if Order_Del.objects.filter(id=order_i.get('id')):   #标识单号已经是存在的内容了,制作内容的更新
            Order_Del.objects.filter(id=order_i.get('id')).update(order_key=order_tou.get('id'), order_code=order_i.get('order_code'),
                                    ...
                                    order_type=order_i.get('order_type'),salesman=order_tou.get('salesman'),
                                    order_level=order_i.get('order_level'),amount=order_i.get('amount'),order_img=order_i.get('order_img'),picture=sku_img)
        else:
    ...
    if order_shen_list: #如果有新增的内容
        Order_Del.objects.bulk_create(order_shen_list)   #保存
    #重新计算画图批次,如果存在的话
    pi_code = Order.objects.filter(id=order_tou.get('id')).values('pi_code')
    if pi_code: #如果存在
        pi_code=pi_code[0].get('pi_code')
        if pi_code:
            Order_Del.objects.filter(order_key=order_tou.get('id')).update(pi_code=pi_code)  # 再次更新批次号码
            pi_list = Order_Del.objects.filter(pi_code=pi_code).values('num')
            num = 0
            for pi_i in pi_list:
                num = num + pi_i.get('num')   #数量合计
            Draw_Pi.objects.filter(id=pi_code).update(total_num=num)
    #重新计算合数,针对同一个ERP有多个单号的情况
    code_list = []  #单号的列表
    order_he_list = Order_Del.objects.filter(order_key=order_tou.get('id')).values('order_code','total_num').distinct() #将所有的ERP单号列出来
    for order_he_i in order_he_list:
        if order_he_i.get('order_code') in code_list:
            break;  #跳过本次循环
        code_list.append(order_he_i.get('order_code'))  #加入列表
        ###干脆所有的合数重新计算一遍
        this_total_num =order_he_i.get('total_num')
        num_i = 0
        order_he_num = Order_Del.objects.filter(order_code=order_he_i.get('order_code')).values('num')
        for ii in order_he_num:
            num_i = num_i+ii.get('num')
        Order_Del.objects.filter(order_code=order_he_i.get('order_code')).update(total_num=num_i)
        if num_i > this_total_num: #如果重新计算的合数大于系统自动计算
            biaozhi=1
    if biaozhi == 0:
        return HttpResponse('OK')
    else:
        return HttpResponse('OJB')
...
def delete_order(request):
    ...
    post_data=request.body  #获得页面数据
    post_data=json.loads(post_data)
    name=post_data.get('name')  #订单号
    status=post_data.get('status')  #状态标识是否有画图批次了
    pi_code=post_data.get('pi_code')  #画图批次
    #先获得订单数据
    order_lists = Order_Del.objects.filter(order_key = name).values('order_code').distinct()  #去重复
    order_lists = list(order_lists)
    #执行删除作业
    if status == 0:  #没有画图
        ...
        Order_Del.objects.filter(order_key=name).delete()
        #重新计算画图批次
        pi_list = Order_Del.objects.filter(pi_code=pi_code).values()
        num = 0
        for pi_i in pi_list:
            num = num +pi_i.get('num')  #数量重新计算
        Draw_Pi.objects.filter(id=pi_code).update(total_num=num)
    #重新计算订单合数--针对一个ERP订单录入两个订单,本订单删除,另外订单的合数需要重新计算
    print(order_lists)
    for order_i in order_lists:
        print(order_i)
        order_code = order_i.get('order_code')
        if Order_Del.objects.filter(order_code=order_code).values('num'): #如果存在这个单号
            num = 0
            orders = Order_Del.objects.filter(order_code=order_code).values('num')  #将每一个ERP单号的明细都列出来
            for orders_i in orders:   #循环计算单个ERP单号的合数
                num = num+orders_i.get('num')
            #更新合数信息
            Order_Del.objects.filter(order_code = order_code).update(total_num=num)
            biaozhi = 1
    if biaozhi == 0:
        return HttpResponse('OK')
    elif biaozhi == 1:
...
def order_Approval(request):
    ...
    #判断批次状态是否要更改
    pi_code = Order.objects.filter(id=order_num).values('pi_code','note')
    if pi_code: #如果存在,可能没有批次呢
        for pi_i in pi_code:
            pi_code = pi_i.get('pi_code')
            note = pi_i.get('note')
        if Order.objects.filter(pi_code=pi_code,status='未审核').values(): #如果存在已经画图,并且状态还是未审核的跳过
            pass
        else: #已经审核
            Draw_Pi.objects.filter(id=pi_code).update(update_status='修改完成',note=note)   #更改状态和备注信息
...
def order_Approval_cancel(request):
    ...
    #获取批次信息,如果存在
    pi_code = Order.objects.filter(id=order_num).values('pi_code')
    if pi_code:
        for pi_i in pi_code:
            pi_code = pi_i.get('pi_code')
        Draw_Pi.objects.filter(id=pi_code).update(update_status='业务员修改中')
    return HttpResponse('OK')
...
def create_pi(request):
    ...
    pi_order = chache[0][0]  #本批次单号
    #3.更新订单单身表信息
    num = 0
    for order_i in pi_list:
        # 再次判断选中的明细行是否已经有了批次信息了
        order_agin = Order_Del.objects.filter(order_key=order_i.get('id')).values()
        for order_j in order_agin:
            if order_j.get('pi_code'):  #如果已经存在批次信息了
                return HttpResponse(order_i.get('id'))
            else:
                pass
        Order_Del.objects.filter(order_key=order_i.get('id')).update(pi_code=pi_order,draw_status='正在画图')
        Order.objects.filter(id=order_i.get('id')).update(draw_status='正在画图',pi_code=pi_order,draw_time=Date)
...
def draw_pi(request):    #获取画图批次信息
    ...
    if user_radio == '3':  # 标识全部订单
        if post_data.get('serch') == '' or post_data.get('serch') is None:
            user_list=Draw_Pi.objects.all().values().order_by('-create_time')
        else:
            ...
            if pi_code:
                ...
                user_list = Draw_Pi.objects.filter(Q(id__icontains=post_data.get('serch'))|Q(create_time__icontains=post_data.get('serch'))
                                                   |Q(id__icontains=pi_code)).values().order_by('-create_time')
            else:
                user_list = Draw_Pi.objects.filter(
                    Q(id__icontains=post_data.get('serch')) | Q(create_time__icontains=post_data.get('serch')) | Q(create_time__icontains=post_data.get('serch'))).values().order_by('-create_time')
    elif user_radio == '2': #标识本地区所有的订单
        if post_data.get('serch') == '' or post_data.get('serch') is None:
            user_list=Draw_Pi.objects.filter(add=user_address).values().order_by('-create_time')
        else:
            user_list = Draw_Pi.objects.filter(add=user_address).values()
            pi_code = Order_Del.objects.filter(Q(order_key__icontains=post_data.get('serch'))|Q(order_code__icontains=post_data.get('serch'))).values('pi_code').distinct()
            if pi_code:
                pi_code=pi_code[0].get('pi_code')  #画图批次获取到了
                user_list = user_list.filter(Q(id__icontains=post_data.get('serch'))|Q(create_time__icontains=post_data.get('serch'))
                                                   |Q(id__icontains=pi_code)).values().order_by('-create_time')
            else:
                user_list = user_list.filter(
                    Q(id__icontains=post_data.get('serch')) | Q(create_time__icontains=post_data.get('serch')) | Q(create_time__icontains=post_data.get('serch'))).values().order_by('-create_time')
        ...
        if post_data.get('serch') == '' or post_data.get('serch') is None:
            user_list = Draw_Pi.objects.filter(create_user=user_code).values().order_by('-create_time')
        else:
            ...
            if pi_code:
                pi_code = pi_code[0].get('pi_code')  # 画图批次获取到了
                user_list = user_list.filter(
                    Q(id__icontains=post_data.get('serch')) | Q(create_time__icontains=post_data.get('serch'))
                    | Q(id__icontains=pi_code)).values().order_by('-create_time')
            else:
                user_list = user_list.filter(
                    Q(id__icontains=post_data.get('serch')) | Q(create_time__icontains=post_data.get('serch')) | Q(
                        create_time__icontains=post_data.get('serch'))).values().order_by('-create_time')
...
def order_complete(request):   #查找状态是画图完成或者是拉图完成的明细
    ...
    user_data = {}
    selectForm = post_data.get('serch')
    if selectForm.get('id') or selectForm.get('order_time') or selectForm.get('draw_time') or selectForm.get('print_time') or selectForm.get('order_code') or selectForm.get('draw_status') or selectForm.get(
            'order_level') or selectForm.get('color') or selectForm.get('words') or selectForm.get('print_status')  or selectForm.get('item_name')\
            or selectForm.get('type') or selectForm.get('order_name') or selectForm.get('salesman') or selectForm.get('sku_style'):
        user_list = Order_Del.objects.filter(Q(draw_status='画图完成')|Q(draw_status='拉图完成')|Q(draw_status='待拉图'))
        if selectForm.get('order_time'): #画图时间
            draw_time = selectForm.get('order_time')
            begin_draw = draw_time[0][:10]
            end_draw = draw_time[1][:10]
            print(selectForm.get('order_time'))
            user_list = user_list.filter(date__gte=begin_draw, date__lte=end_draw)
        ...
        if selectForm.get('print_time'): #打印时间
        ...
        if selectForm.get('order_level'):
            user_list = user_list.filter(order_level=selectForm.get('order_level'))
        if selectForm.get('sku_style'):
            user_list = user_list.filter(sku_style=selectForm.get('sku_style'))
        if selectForm.get('color'):
            user_list = user_list.filter(color=selectForm.get('color'))
        if selectForm.get('words'):
            user_list = user_list.filter(words=selectForm.get('words'))
        ...
        if selectForm.get('salesman'):
            user_list = user_list.filter(Q(salesman__contains=selectForm.get('salesman')) | Q(draw_user__contains=selectForm.get('salesman')))
    ...
    total = Paginator(user_list, post_data.get('size')).count  # 总计的数量
    pages = Paginator(user_list, post_data.get('size'))
    pages_num = pages.num_pages   #
    #print(Paginator(user_list, post_data.get('size')).num_pages,Paginator(user_list, post_data.get('size')).page_range  )
    if pages_num < post_data.get('page') :  #如果总数量小于当前的页数
        user_page = pages.page(1)   #默认就第一页
    else:
        user_page = pages.page(post_data.get('page'))
    user_data['user_list'] = list(user_page)
    user_data['total'] = total
    return JsonResponse(data=user_data, safe=False)
def draw_agin(request):
    ...
    return HttpResponse('OK')
def draw_agin_end(request):
    post_data = request.body  # 获得页面数据
    post_data = json.loads(post_data)
    pi_list = post_data.get('data')
    #.更新订单单身表信息,将画图完成,更新为拉图完成
    for order_i in pi_list:
        Order_Del.objects.filter(id=order_i.get('id')).update(draw_status='拉图完成')
    return HttpResponse('OK')
def draw_again_del(request):
    ...
    user = post_data.get('user')
    if selectForm.get('date') or selectForm.get('order_code') or selectForm.get('draw_status') or selectForm.get('order_level') or selectForm.get('color') or selectForm.get('words')\
            or selectForm.get('type') or selectForm.get('order_name') or selectForm.get('salesman') or selectForm.get('draw_user'):
        user_list=Order_Del.objects.filter(draw_status='待拉图')
        if selectForm.get('date'):
            draw_time = selectForm.get('date')
            begin_draw = draw_time[0][:10]
            end_draw = draw_time[1][:10]
        ...
        if selectForm.get('draw_user'):
            user_list = user_list.filter(draw_user__contains=selectForm.get('draw_user'))
        user_list=user_list.values().order_by('-id')   #转换取值

    else:
        user_list = Order_Del.objects.filter(draw_status='待拉图').values().order_by('-id')
    user_page = Paginator(user_list, post_data.get('size')).page(post_data.get('page'))
    total = Paginator(user_list, post_data.get('size')).count  # 总计的数量
    for user_i in user_page:
        user_json.append(user_i)
...
def preview_order(request):   #查找状态是画图完成或者是拉图完成的明细
    post_data = request.body
    post_data = json.loads(post_data)
    order_num = post_data.get('order_num')
    user_list = Order_Del.objects.filter(order_key=order_num).values().order_by('-id')
...
def warehouse(request):    #获取库存明细
    ...
    user_data['total']=total
    return JsonResponse(data=user_data,safe=False)
def warehouse_out(request):    #获取出库明细
    #有权限区分,不同的人看到订单内容是不一样的
    post_data = request.body
    post_data = json.loads(post_data)
    user_json = []
    user_data={}
    if post_data.get('serch') == '' or post_data.get('serch') is None:
        user_list=Warehousing_Out.objects.filter().values().order_by('-id')
    else:
        user_list = Warehousing_Out.objects.filter(Q(id__icontains=post_data.get('serch'))|Q(date__icontains=post_data.get('serch'))|Q(note__icontains=post_data.get('serch'))).values().order_by('-id')
    user_page=Paginator(user_list,post_data.get('size')).page(post_data.get('page'))
    total= Paginator(user_list,post_data.get('size')).count    #总计的数量
    for user_i in user_page:
        user_json.append(user_i)
...
def update_draw(request):
    ...
    if len(complete_time) == 10:
        complete_time=complete_time+' '+'00:00'
    aa=Draw_Pi.objects.filter(id=id).update(draw_user=draw_user,complete_time=complete_time)
    #修改订单明细的画图人信息
    Order_Del.objects.filter(pi_code=id).update(draw_user=draw_user,draw_time=complete_time)
    return HttpResponse('OK')
def refund_order(request):
    post_data = request.body
    post_data = json.loads(post_data)
    id = post_data.get('id')
    Order_Del.objects.filter(id=id).update(refund='退款',print_status='已打印')
    return HttpResponse('OK')
def refund_cancel(request):
    post_data = request.body
    post_data = json.loads(post_data)
    id = post_data.get('id')
    Order_Del.objects.filter(id=id).update(refund='')
    return HttpResponse('OK')
def rework_cancel(request):
    post_data = request.body
    post_data = json.loads(post_data)
    id = post_data.get('id')
    Order_Del.objects.filter(id=id).update(Rework='')
    return HttpResponse('OK')
def rework_order(request):
    post_data = request.body
    post_data = json.loads(post_data)
    id = post_data.get('id')
    Order_Del.objects.filter(id=id).update(Rework='返工')
    return HttpResponse('OK')
def ruku_print(request):
    post_data = request.body
    post_data = json.loads(post_data)
    order_num = post_data.get('order_num')  #得到用户账号
    #print(name)
    if order_num :
        rukus = Warehous_Del.objects.filter(order_key=order_num).values()
        ruku_list = list(rukus)
        return JsonResponse(data=ruku_list, safe=False)
def Stock_account(request):
    post_data = request.body
    post_data = json.loads(post_data)
    user_data = {}
    if post_data.get('serch') == '' or post_data.get('serch') is None:
        user_list1 = Warehous_Del.objects.all().values('order_key','item_code','item_name','format','unit','date','num')
        user_list2 = Warehous_Del_Out.objects.all().values('order_key','item_code','item_name','format','unit','date','num')
        #user_list = chain(user_list1, user_list2)
        user_list = user_list1.union(user_list2)
    else:
        user_list1 = Warehous_Del.objects.filter(Q(item_code__icontains=post_data.get('serch')) | Q(item_name__icontains=post_data.get('serch'))).values('order_key','item_code','item_name','format','unit','date','num')
        user_list2 = Warehous_Del_Out.objects.filter(Q(item_code__icontains=post_data.get('serch')) | Q(item_name__icontains=post_data.get('serch'))).values('order_key','item_code','item_name','format','unit','date','num')
        #user_list = chain(user_list1, user_list2)
        user_list = user_list1.union(user_list2)
    user_page = Paginator(user_list, post_data.get('size')).page(post_data.get('page'))
    total = Paginator(user_list, post_data.get('size')).count  # 总计的数量
